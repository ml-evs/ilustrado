...
def fitness_swarm_plot(generations, ax=None, save=False):
    ...
    sns.set_palette("Dark2", desat=0.5)
    if ax is None:
        fig = plt.figure(figsize=(10, 8))
        ax = fig.add_subplot(111)
    fitnesses = np.asarray(
        [generation.raw_fitnesses for generation in generations if len(generation) > 1]
    ).T
    ...
    sns.swarmplot(
        ...
        ax=ax,
        linewidth=1,
        palette=sns.color_palette("Dark2", desat=0.5),
    )
...
def plot_new_2d_hull(generations, hull, points=True, label_hull=True, save=True):
    ...
    for structure in generations[0].populace:
        ax.scatter(
            structure["concentration"][0],
            structure["formation_enthalpy_per_atom"],
            c="r",
            s=50,
            zorder=100000000,
            lw=2,
    ...
    for idx, generation in enumerate(generations[1:]):
        for structure in generation.populace:
            if "mutations" not in structure:
                ...
                colour = (
                    mutant_colours[idx]
                    if "crossover" not in structure["mutations"]
                    else crossover_colours[idx]
                )
            ax.scatter(
                structure["concentration"][0],
                structure["formation_enthalpy_per_atom"],
